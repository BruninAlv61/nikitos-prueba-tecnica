...
@section('contenido')
    ...
    <div class="form-card form-card--wide">
        <form action="{{ route('admin.recetas.update', $receta->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label for="receta_titulo">Título</label>
                <input id="receta_titulo" type="text" name="titulo" value="{{ old('titulo', $receta->titulo) }}" maxlength="255" required autocomplete="off">
            ...
            <div class="form-group">
                ...
                @if($receta->imagen)
                    <img src="{{ $receta->imagen_url }}" class="thumb" style="margin-bottom:8px;width:80px;height:80px;object-fit:cover;border-radius:6px;" alt="{{ $receta->titulo }}"><br>
                @endif
                <label for="receta_imagen">Nueva imagen</label>
            ...
            <div class="admin-form-grid">
                <div class="form-group">
                    ...
                    <input id="receta_tiempo" type="text" name="tiempo_preparacion"
                           value="{{ old('tiempo_preparacion', $receta->tiempo_preparacion) }}"
                           maxlength="100" placeholder="Ej: 30 minutos" autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <input id="receta_porciones" type="number" name="porciones"
                           value="{{ old('porciones', $receta->porciones) }}" min="1" max="500">
            ...
            <div class="form-group">
                ...
                <div id="ingredientes-lista" style="display:flex;flex-direction:column;gap:8px;">
                    @foreach(old('ingredientes', $receta->ingredientes ?? ['']) as $ing)
                ...
                @error('ingredientes.*')<span class="form-error" role="alert">{{ $message }}</span>@enderror
                <button type="button"
                        onclick="agregarCampo('ingredientes-lista','ingredientes[]','Ej: 1 taza de agua')"
            ...
            <div class="form-group">
                ...
                <div id="pasos-lista" style="display:flex;flex-direction:column;gap:8px;">
                    @foreach(old('pasos', $receta->pasos ?? ['']) as $paso)
