...
<header class="header">
    <a href="{{ route('home') }}" class="header__logo-link" aria-label="Nikitos — inicio">
        <img class="logo" src="{{ asset('images/logo.png') }}" alt="Nikitos snacks">
    </a>

    @auth
        @include('partials.user-account-menu', ['menuSuffix' => 'desktop'])
    @else
        <a href="{{ route('login') }}" class="button-login--desktop">
            Ingresar
